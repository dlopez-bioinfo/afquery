...
def _compute_chunk_annotations(
    ...
    sf: SampleFilter,
) -> dict[tuple[int, str, str], tuple[int, int, bool, int | None, int, int, int]]:
    ...
    N_FAIL is None for v1 DBs without fail_bitmap.
    """
    from pathlib import Path
    from .query import QueryEngine

    engine = QueryEngine(db_path)
    has_fail = engine._has_fail_bitmap
    ...
    variant_data: dict[tuple[int, str, str], tuple[bytes, bytes, bytes | None]] = {}
    _db = Path(db_path)
    bucket_start = bucket_id * 1_000_000
    bucket_end = (bucket_id + 1) * 1_000_000 - 1
    _fail_sel = ", fail_bitmap" if has_fail else ""
        ...
        if valid_positions and parquet_file.exists():
            ...
            for row in rows:
                ...
                variant_data[(pos, ref, alt)] = (bytes(het_bytes), bytes(hom_bytes),
                                                  bytes(fail_bytes) if fail_bytes is not None else None)
    else:
    ...
    result: dict[tuple[int, str, str], tuple[int, int, bool, int | None, int, int, int]] = {}
    for pos, ref, alts in records:
        ...
        for alt in alts:
            ...
            if key in result:
                continue  # dedup
            if AN == 0:
                result[key] = (0, 0, False, None, 0, 0, 0)
                ...
                N_FAIL: int | None = len(deserialize(fail_bytes) & eligible) if fail_bytes is not None else None
                N_HOM_REF = len(eligible) - N_HET - N_HOM_ALT - (N_FAIL or 0)
                result[key] = (AC, AN, True, N_FAIL, N_HET, N_HOM_ALT, N_HOM_REF)
            else:
                result[key] = (0, AN, False, None, 0, 0, len(eligible))
...
def annotate_vcf(
    ...
    for (chrom, bucket) in work_order:
        ...
        for variant in variant_buffers[(chrom, bucket)]:
            ...
            if has_fail_data and engine._has_fail_bitmap:
