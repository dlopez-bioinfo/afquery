...
**Returns:** Stats dict with keys `n_annotated`, `n_skipped`, `elapsed_seconds`.
...
Validate database integrity. Returns a list of error messages (empty = healthy).
...
Return changelog history. Each entry has `version`, `timestamp`, `operation`, `description`.
...
class QueryResult:
    ...
    N_FAIL: int              # Samples with FILTER≠PASS (schema v2; 0 for v1)
```
